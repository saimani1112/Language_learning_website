            ...
            <div class="row g-4">
                ...
                <div class="col-lg-3 col-sm-6 wow fadeInUp" data-wow-delay="0.5s">
                    <div class="service-item text-center pt-3">
                        <div class="p-4">
                            <i class="fa fa-3x fa-home text-primary mb-4"></i>
                            <a href=""><h5 class="mb-3">Greetings and Farewell</h5></a>
                        </div>
                    </div>
                </div>
...
// Insert operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_insert_frequently_used_words"])) {
    $quizID = $_POST["quiz_id"];
    $languageID = $_POST["language_id"];
    $wordsInLang = $_POST["words_in_language"];
    $englishMeaning = $_POST["english_meaning"];

    $sql = "INSERT INTO frequently_used_words (quiz_id, language_id, words_in_language, english_meaning) 
            VALUES ('$quizID', '$languageID', '$wordsInLang', '$englishMeaning')";

    if ($conn->query($sql) === TRUE) {
        echo "New frequently used words added successfully";
    } else {
...
// Delete operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_delete_frequently_used_words"])) {
    $quizIDToDelete = $_POST["quiz_id_to_delete"];

    $sql = "DELETE FROM frequently_used_words WHERE quiz_id = '$quizIDToDelete'";

    if ($conn->query($sql) === TRUE) {
        echo "Frequently used words deleted successfully";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}
...
// Update operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_update_frequently_used_words"])) {
    ...
    $sql = "UPDATE frequently_used_words 
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Frequently used words updated successfully";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}

// Fetch data from the frequently_used_words table
$sql = "SELECT * FROM frequently_used_words";
$result = $conn->query($sql);
?>

<div class="container">
    <h6 class="section-title bg-white text-start text-primary pe-3">Admin Dashboard</h6>
    <h1 class="mb-4">Frequently Used Words</h1>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_insert_frequently_used_words" value="Insert">
    </form>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_delete_frequently_used_words" value="Delete">
    </form>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_update_frequently_used_words" value="Update">
    </form>

    <!-- Display frequently_used_words table -->
    <table>
        <tr>
            <th>Quiz ID</th>
            <th>Language ID</th>
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<tr><td colspan='4'>No frequently used words found</td></tr>";
        }
        ?>
    </table>
</div>
