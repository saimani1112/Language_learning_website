...
// If the user has selected languages, fetch travel information for each language
if ($userLanguagesResult->num_rows > 0) {
    ...
    <div class="container">
        ...
        while ($userLanguage = $userLanguagesResult->fetch_assoc()) {
            ...
            $travelQuery = "SELECT * FROM travel WHERE language_id = '$userLanguageID'";
            $travelResult = $conn->query($travelQuery);

            echo "<h2>{$userLanguageName}</h2>";

            // Display travel table for the current language
            if ($travelResult->num_rows > 0) {
                ...
                echo "<p>No travel entries found for {$userLanguageName}</p>";
            }
        }
        ?>
