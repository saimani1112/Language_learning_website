...
$userLanguageQuery = "SELECT ul.language_id FROM user_lang ul
                    JOIN user u ON ul.userid = u.userid
                    JOIN Language l ON ul.language_id = l.language_id
                    WHERE u.email = '$email'";
$userLanguageResult = $conn->query($userLanguageQuery);

$userLanguage = null;

// Check if the query was successful and rows were returned
if ($userLanguageResult && $userLanguageResult->num_rows > 0) {
    // Store the language_id in the variable
    $userLanguage = $userLanguageResult->fetch_assoc()['language_id'];
}


// Check if the form is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($userLanguageResult->num_rows > 0) {
        //$userLanguage = $userLanguageResult->fetch_assoc()['language_id'];
        $quizAnswers = $_POST['quiz_answers'];

        // Calculate the score
        $score = 0;
        foreach ($quizAnswers as $questionID => $userAnswer) {
            $questionID = mysqli_real_escape_string($conn, $questionID);
            $userAnswer = mysqli_real_escape_string($conn, $userAnswer);

            $quizQuery = "SELECT answer FROM quiz WHERE quiz_id = '$questionID' AND language_id = '$userLanguage'";
            $quizResult = $conn->query($quizQuery);

            if ($quizResult->num_rows > 0) {
                $correctAnswer = $quizResult->fetch_assoc()['answer'];
        ...
        // Update or insert the score into the quiz_score table
        $userID = $userInfo['userid'];
        $scoreQuery = "UPDATE quiz_score SET score = $score WHERE userid = '$userID' AND language_id = '$userLanguage'";
        $conn->query($scoreQuery);

        echo "<center><p>Your score is: $score</p><center>";
    } else {
        echo "<p>User has not selected a language.</p>";
    }
}

// Display quiz questions
$quizQuery = "SELECT * FROM quiz WHERE language_id = '$userLanguage'";
$quizResult = $conn->query($quizQuery);
...
if ($quizResult->num_rows > 0) {
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <h6 class="section-title bg-white text-start text-primary pe-3">Quiz</h6>
        <?php
        while ($row = $quizResult->fetch_assoc()) {
            $questionID = $row['quiz_id'];
            $question = $row['question'];
            echo "<p>$question</p>";
            echo "<input type='text' name='quiz_answers[$questionID]' required><br>";
        }
        ?>
...
<?php
    ...
    echo "<p>No quiz questions found for the selected language.</p>";
}
