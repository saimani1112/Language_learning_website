...
// If the user has selected languages, fetch greetings_and_farewell entries for each language
if ($userLanguagesResult->num_rows > 0) {
    ...
    <div class="container">
        ...
        while ($userLanguage = $userLanguagesResult->fetch_assoc()) {
            ...
            $greetingsQuery = "SELECT * FROM greetings_and_farewell WHERE language_id = '$userLanguageID'";
            $greetingsResult = $conn->query($greetingsQuery);

            echo "<h2>{$userLanguageName}</h2>";

            // Display greetings_and_farewell table for the current language
            if ($greetingsResult->num_rows > 0) {
                echo "<table>";
                echo "<tr><th>Words in Language</th><th>English Meaning</th></tr>";
                while ($row = $greetingsResult->fetch_assoc()) {
                    echo "<tr>";
                ...
                echo "<p>No greetings_and_farewell entries found for {$userLanguageName}</p>";
            }
        }
        ?>
