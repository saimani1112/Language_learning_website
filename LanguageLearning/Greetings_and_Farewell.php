...
// Insert operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_insert_greetings"])) {
    $languageID = $_POST["language_id"];
    $quizID = $_POST["quiz_id"];
    $wordsInLang = $_POST["words_in_language"];
    $englishMeaning = $_POST["english_meaning"];

    $sql = "INSERT INTO greetings_and_farewell (language_id, quiz_id, words_in_language, english_meaning) 
            VALUES ('$languageID', '$quizID', '$wordsInLang', '$englishMeaning')";

    if ($conn->query($sql) === TRUE) {
        echo "New greetings and farewell added successfully";
    } else {
...
// Delete operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_delete_greetings"])) {
    $quizIDToDelete = $_POST["quiz_id_to_delete"];

    $sql = "DELETE FROM greetings_and_farewell WHERE quiz_id = '$quizIDToDelete'";

    if ($conn->query($sql) === TRUE) {
        echo "Greetings and farewell deleted successfully";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}
...
// Update operation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_update_greetings"])) {
    ...
    $sql = "UPDATE greetings_and_farewell 
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Greetings and farewell updated successfully";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}

// Fetch data from the greetings_and_farewell table
$sql = "SELECT * FROM greetings_and_farewell";
$result = $conn->query($sql);
?>

<div class="container">
    <h6 class="section-title bg-white text-start text-primary pe-3">Admin Dashboard</h6>
    <h1 class="mb-4">Greetings and Farewell</h1>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_insert_greetings" value="Insert">
    </form>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_delete_greetings" value="Delete">
    </form>
    ...
    <form method="post" action="">
        ...
        <input type="submit" name="submit_update_greetings" value="Update">
    </form>

    <!-- Display greetings_and_farewell table -->
    <table>
        <tr>
            <th>Language ID</th>
            <th>Quiz ID</th>
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<tr><td colspan='4'>No greetings and farewell found</td></tr>";
        }
        ?>
    </table>
</div>
